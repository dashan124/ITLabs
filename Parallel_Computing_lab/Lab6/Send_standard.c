...
int main(int argc, char const *argv[])
{
	...
	if(myrank==0)
	{
		...
		MPI_Send(x,5,MPI_INT,1,1,MPI_COMM_WORLD); //Blocking send will expect
		//matching receive at the destination
		//In Standard mode,Send will return after copying the data to the buffer
		MPI_Send(y,5,MPI_INT,1,2,MPI_COMM_WORLD);// This send will be initiated
		//and matching receive is already there so the program will not lead to deadlock
	}
	else if(myrank==1)
	{
		MPI_Recv(x,5,MPI_INT,0,2,MPI_COMM_WORLD,&status); //P1 will block as it
		//has not received a matching send with tag 2
		for(i=0;i<5;i++)
			printf("Received Array x : %d\n",x[i]);
			MPI_Recv(y,5,MPI_INT,0,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		for(i=0;i<5;i++)
