...
int main(){
  ...
  for(int x=0;x<n;x++){
    for(int y=0;y<n-x-1;y++){
      if(proc[y].Burst_time>proc[y+1].Burst_time){
        struct Process temp;
        temp=proc[y];
        proc[y]=proc[y+1];
        proc[y+1]=temp;
  ...
  for(int x=0;x<n;x++){
    for(int y=0;y<n;y++){
      if(proc[x].Arrival_time==proc[y].Arrival_time){
        if(proc[x].Burst_time>proc[y].Burst_time){
          struct Process temp;
          temp=proc[x];
          proc[x]=proc[y];
          proc[y]=temp;
  ...
  printf("\n");
  printf("\nProcesses\t Arrival \t Burst time \t Waiting time \t Turn around time\n");
  for (int i = 0; i < n; i++)
  {
      total_wt = total_wt + waiting[i];
      total_tat = total_tat + tat[i];
      printf(" %d \t\t %d  \t\t %d  \t\t %d  \t\t%d  \t\n",proc[i].processId,proc[i].Arrival_time,proc[i].Burst_time,waiting[i],tat[i]);
  }
  printf("Average Waiting time =  %lf\n",(float)total_wt / (float)n );
  printf("Average Waiting time =  %lf\n",(float)total_tat / (float)n);
